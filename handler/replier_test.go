...
func (test *TestReplierSuite) req(client *zmq.Socket, request message.Request) message.ReplyInterface {
	...
	reqStr, err := test.replier.Packer().SerializeRequest(&request)
	s.Require().NoError(err)
	...
	reply, _, err := test.replier.Packer().DeserializeReply(raw)
	s.Require().NoError(err)
...
func (test *TestReplierSuite) Test_10_StartHandlesRequests() {
	...
	s.Require().Equal(SocketReady, test.replier.Control.Status())
